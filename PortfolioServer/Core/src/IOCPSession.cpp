...
void IOCPSession::HandleStream(Stream& stream)
{
    ...
    while (offset + sizeof(PacketHeader) <= totalSize)
    {
        ...
        auto const packetId = pktHeader->_id;
        auto const packetBodySize = pktHeader->_size;

        auto const packetBodyStart = offset + static_cast<uint32_t>(sizeof(PacketHeader));
        auto const packetBodyEnd = packetBodyStart + packetBodySize;
        ...
        auto const* payload = data + packetBodyStart;
