...
public:
    ...
    {
        auto const session = IOCPSessionManager::Singleton::GetConstInstance().Find(_sessionId);
        if (not session)
        {
            return;
        }
...
public:
    ...
    void Execute() override
    {
        auto const iocpSession = IOCPSessionManager::Singleton::GetConstInstance().Find(_sessionId);
        if (not iocpSession)
        {
            return;
        }
